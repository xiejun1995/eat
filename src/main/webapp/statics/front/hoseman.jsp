...
<%--			<div id="content" class="span10">--%>
	...
	<script>
		$(function () {
			$.getJSON("${pageContext.request.contextPath}/order/findUnOrder",function (result) {
				var $url = $("tbody");
				var td = "";
				// var $result = result.result;
				$(result).each(function (i) {
					var merchantName = result[i].merchantName;
					var merchantPhone = result[i].merchantPhone;
					var merchantAddress = result[i].merchantAddress;
					var orderTime = result[i].orderTime;
					var orderDeliveryTime = result[i].orderDeliveryTime;
					var userName = result[i].userName;
					var userPhone = result[i].userPhone;
					var userAddress = result[i].userAddress;
					td += "<tr><td>"+merchantName+"</td><td>"+merchantPhone+"</td><td>"+merchantAddress+"</td><td>"+orderTime+"</td><td>"+orderDeliveryTime+"</td><td>"+
							userName+"</td><td>"+userPhone+"</td><td>"+userAddress+"</td></tr>";
				});
				$url.html(td);
			});
		});
